...
import torch
from md.loader_expert import All_Dataset
from md.model_expert import AlexNet as Model
import numpy as np

train_dataset = All_Dataset("./cifar_10/train/",
                            transform = Compose([ToTensor()]))

train_ld = DataLoader(train_dataset,shuffle=True)

test_dataset = All_Dataset("./cifar_10/test/",transform = Compose([ToTensor()]))
...
torch.save(model.state_dict(),"./models/basic/basic.pth")
...
model = Model(10).to(device).float()
model.load_state_dict(torch.load("./models/basic/basic.pth"))
